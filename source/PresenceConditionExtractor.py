...
class PresenceConditionExtractor:
    ...
    def extract_pc_from_caller_context(self, caller_name: str, callee_name: str) -> List[str]:
        """
        Extract all presence conditions (PCs) under which the callee is called from the caller.
        """
        ...
        call_lines = self._find_calls_within_bounds(callee_name, caller_start, caller_end)
        if not call_lines:
            return ["CALL_NOT_FOUND"]

        pcs = []
        for line_no in call_lines:
            pc = self._resolve_pc_from_blocks(caller_start, line_no)
            pcs.append(pc)
        return pcs
    ...
    def _find_calls_within_bounds(self, callee: str, start: int, end: int) -> List[int]:
        ...
        return [i for i in range(start, end + 1) if pattern.search(self.source_code_lines[i])]
